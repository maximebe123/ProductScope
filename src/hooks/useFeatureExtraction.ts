/**
 * Hook for managing feature extraction from GitHub
 * Extracts EXISTING features from a codebase (unlike discovery which suggests new features)
 */
...
interface UseFeatureExtractionResult {
  state: ExtractionState
  startExtraction: (projectId: string, options: FeatureExtractionRequest) => Promise<void>
...
export function useFeatureExtraction(): UseFeatureExtractionResult {
  ...
  const startExtraction = useCallback(async (
    ...
    try {
      for await (const event of featureApi.extractFromGitHub(
        projectId,
        options,
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        setState(prev => ({ ...prev, phase: 'idle', error: 'Extraction cancelled' }))
      } else {
        setState(prev => ({
          ...prev,
          phase: 'error',
  ...
  const handleEvent = useCallback((event: FeatureExtractionEvent) => {
    switch (event.type) {
      case 'agent_start':
        setState(prev => ({
          ...prev,
